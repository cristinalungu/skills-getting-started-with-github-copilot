      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = (details.participants && details.participants.length > 0)
          ? `<ul class="participants-list">${details.participants
              .map(p => `<li><span class="participant-name">${p}</span><button class="delete-btn" data-activity="${name}" data-participant="${p}" title="Remove participant">🗑</button></li>`)
              .join("")}</ul>`
        ...
        deleteButtons.forEach(button => {
          button.addEventListener("click", async (e) => {
            ...
            await removeParticipant(activity, participant);
          });
        });
